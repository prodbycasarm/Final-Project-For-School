...
<body>
    ...
    // Fetch list of employers (milieu de stage)
    $employeurs_query = "SELECT noemployeur, `Nom de l'employeur`, `Nom de l'entreprise` FROM acces_employeurs";
    $employeurs_result = mysqli_query($con, $employeurs_query);
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        // Validate the form data
        if (empty($nometu) || empty($nomemployeur) || empty($nomsuperviseur)) {
            $message = 'Tous les champs sont requis.';
        } else {
            // Update the data into the database
            $query = "UPDATE acces_etu SET noemployeur = ?, nomsup = ? WHERE numetu = ?";
            $stmt = mysqli_prepare($con, $query);
            mysqli_stmt_bind_param($stmt, 'sss', $nomemployeur, $nomsuperviseur, $nometu);

            if (mysqli_stmt_execute($stmt)) {
                $message = 'Étudiant associé au milieu de stage et superviseur avec succès !';
            } else {
        ...
        $nometu = $_GET['numetu'];
        $query = "SELECT numetu, noemployeur, nomsup FROM acces_etu WHERE numetu = ?";
        $stmt = mysqli_prepare($con, $query);
        mysqli_stmt_bind_param($stmt, 's', $nometu);
        mysqli_stmt_execute($stmt);
        mysqli_stmt_bind_result($stmt, $current_nometu, $current_nomemployeur, $current_nomsuperviseur);
...
<div class="tableau">
    <h2>Associer un étudiant à un milieu de stage et un superviseur</h2>

    <?php if (!empty($message)): ?>
        <p style="color: <?php echo (strpos($message, 'succès') !== false) ? 'green' : 'red'; ?>;"><?php echo $message; ?></p>
    <?php endif; ?>

    <form action="admin_associer_multiple.php" method="POST" id="createStudentForm">
        <div style="float:left; margin-left:30px;">
            ...
            <label for="nomemployeur">Choisir un milieu de stage :</label><br />
            <select id="nomemployeur" name="nomemployeur" style="float:left; margin-bottom:19px;">
                ...
                <?php while ($row = mysqli_fetch_assoc($employeurs_result)): ?>
                    <option value="<?php echo htmlspecialchars($row['noemployeur']); ?>"
                        <?php echo ($row['noemployeur'] == $current_nomemployeur) ? 'selected' : ''; ?>>
        ...
        <div>
            ...
            $sql = "SELECT 
                        acces_etu.nometu, 
                        acces_etu.nomsup AS Superviseur, 
                ...
                <table style="border-collapse: collapse; width: 70%; margin: 0 auto;">
                    <thead>
                        <tr>
                            ...
                            <th style="border: 1px solid black; padding: 8px; text-align: center;">Milieu de Stage</th>
                        ...
                        while ($row = mysqli_fetch_assoc($result)) {
                            echo "<tr>
                                    ...
                                    <td style='border: 1px solid black; padding: 8px; text-align: center;'>" . 
                                        htmlspecialchars($row['NomEmployeur']) . " - " . htmlspecialchars($row['NomEntreprise']) . 
                                    "</td>

                                    <td style='border: 1px solid black; padding: 8px; text-align: center;'>" . htmlspecialchars($row['Superviseur']) . "</td>
                                </tr>";
                        }
                        ?>
