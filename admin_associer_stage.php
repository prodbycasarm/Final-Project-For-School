...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validate the form data
    if (empty($nometu) || empty($nomemployeur)) {
        $message = 'Tous les champs sont requis.';
    } else {
        // Update the data into the database
        $query = "UPDATE acces_etu SET noemployeur = ? WHERE numetu = ?";
        $stmt = mysqli_prepare($con, $query);
        mysqli_stmt_bind_param($stmt, 'ss', $nomemployeur, $nometu);

        if (mysqli_stmt_execute($stmt)) {
            $message = 'Étudiant associé au milieu de stage avec succès !';
        } else {
    ...
    $nometu = $_GET['numetu'];
    $query = "SELECT numetu, noemployeur FROM acces_etu WHERE numetu = ?";
    $stmt = mysqli_prepare($con, $query);
    mysqli_stmt_bind_param($stmt, 's', $nometu);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_bind_result($stmt, $current_nometu, $current_nomemployeur);
...
<div class="tableau">
    ...
    <form action="admin_associer_stage.php" method="POST" id="createStudentForm">
    ...
    <div>          
        ...
        <?php
        include "bd.php";
        // Corrected SQL query (removes supervisor)
        $sql = "SELECT 
                    acces_etu.nometu, 
