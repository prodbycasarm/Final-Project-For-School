...
<head>
    ...
    <title>Rapports de Stage - 2023 Employeur</title>
    <style>
         * {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nomuser) || empty($numuser) || empty($nomDeLEntreprise) || empty($motpasse) || empty($motpasse2)) {
        ...
        if (mysqli_num_rows($result) > 0) {
            $message = 'Le numéro de l\'employeur existe déjà.';
        } else {
    
   
            // Create the email based on the student's name
            $createmail = "$nomuser@$nomDeLEntreprise.com";
            $createmail = strtolower($createmail);
            
            
            
            // Insert the new employer into the database
            $query = "INSERT INTO acces_employeurs (`Nom de l'employeur`, noemployeur, `Nom de l'entreprise`, mdpemployeur, courriel) VALUES (?, ?, ?, ?, ?)";
            $stmt = mysqli_prepare($con, $query);
            mysqli_stmt_bind_param($stmt, 'sssss', $nomuser, $numuser, $nomDeLEntreprise, $motpasse, $createmail);

            if (mysqli_stmt_execute($stmt)) {
                $message = 'L\'employeur à été créé avec succès !';
            } else {
...
<body>
    <div class="main">
        ...
        <div class="tableau">
            <h2>Créer un employeur</h2>
            <?php if (!empty($message)): ?>
                <p style="color: <?php echo (strpos($message, 'succès') !== false) ? 'green' : 'red'; ?>;"><?php echo $message; ?></p>
            <?php endif; ?> 
            <form id="createStudentForm" action="admin_employeur.php" method="post" onsubmit="return validateForm()">
                <div style="float:left;">
                    <label for="nomuser">Nom de l'employeur</label><br />
                    <input class="questions" type="text" id="nomuser" name="nomuser" value="" placeholder="Prénom et nom" maxlength="35" /><br />
                    <label for="numuser">Numéro de l'employeur</label><br />
                    <input class="questions" type="text" id="numuser" name="numuser" value="" placeholder="####" maxlength="4" /><br />
                    <label for="Nomdelentreprise">Nom de l'entreprise</label><br />
                    <input class="questions" type="text" id="Nomdelentreprise" name="Nomdelentreprise" value="" placeholder="Compagnie"  /><br />
        ...
        <script>
            ...
            function validateForm() {
                ...
                var Nomdelentreprise = document.getElementById("Nom de l\'entreprise").value;
        ...
        <div class="tableau">
            <h2>Modifier/Supprimer un employeur</h2>
            ...
            <?php
                ...
                $sql = "SELECT `Nom de l'employeur`, noemployeur, mdpemployeur, `Nom de l'entreprise` FROM acces_employeurs";
            ...
            <div>
                <table style="border-collapse: collapse; width: 70%; margin: 0 auto;">
                    <thead>
                        <tr>
                            <th style="border: 1px solid black; padding: 8px; text-align: center;">Numéro d'employeur</th>
                            <th style="border: 1px solid black; padding: 8px;">Nom de l'employeur</th>
                            <th style="border: 1px solid black; padding: 8px;">Mot de passe</th>
                            <th style="border: 1px solid black; padding: 8px;">Entreprise</th>
                    ...
                    <tbody>
                        ...
                        while ($row = mysqli_fetch_assoc($result)) {
                            ...
                            $modifier_link = "modifier_employeur.php?num=" . urlencode($row['noemployeur']);
                            $delete_link = "supprimer_employeur.php?num=" . urlencode($row['noemployeur']);
                        ?>
                            <tr>
                                <td style="border: 1px solid black; padding: 8px; text-align: center;">
                                    <?php echo htmlspecialchars($row['noemployeur']); ?>
                                ...
                                <td style="border: 1px solid black; padding: 8px;">
                                    <?php echo htmlspecialchars($row['mdpemployeur']); ?>
                                </td>
                                <td style="border: 1px solid black; padding: 8px;">
                                ...
                                <td style="border: 1px solid black; padding: 8px; text-align: center;">
                                    ...
                                    <a href="<?php echo $modifier_link; ?>" title="Modifier l'employeur " >
                                        Modifier
                                    </a>

                                    <!-- Lien pour supprimer le rapport -->
                                    <a href="<?php echo $delete_link; ?>" title="Supprimer l'employeur" 
                                    onclick="return confirm('Êtes-vous sûr de vouloir supprimer l&#39;employeur ?');">
                                        Supprimer
                                    </a>
